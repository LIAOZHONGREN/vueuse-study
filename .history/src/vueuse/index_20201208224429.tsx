...
import {
    ...
    asyncComputed, useAsyncState, useRefHistory, biSyncRef, controlledComputed, extendRef, makeDestructurable, syncRef, useDebounce, useDebounceFn
...
const UseRefHistory = defineComponent({
    setup() {
        ...
        return () => (
            <div>
                <span>useRefHistory:跟踪参数的更改历史，提供撤消和还原功能</span><br />
                <button onClick={() => { randomNumRef.value = Math.floor(Math.random() * 100) }}>产生随机数</button><br />
                <button onClick={undo} disabled={!canUndo.value}>撤消</button>
                <button onClick={redo} disabled={!canRedo.value}>还原</button><br />
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {
...
const BiSyncRef = defineComponent({
    setup() {
        const [aRef, bRef] = [ref(''), ref('')]
        const stop = biSyncRef(aRef, bRef)
        return () => (
            <div>
                <span>biSyncRef:双向引用同步</span><br />
                <input type="text" value={aRef.value} onInput={e => { aRef.value = e.target.value }} placeholder='A' /><br />
                <input type="text" value={bRef.value} onInput={e => { bRef.value = e.target.value }} placeholder='B' /><br />
...
const ControlledComputed = defineComponent({
    setup() {
        const sourceRef = ref(0)
        const comRef = controlledComputed(sourceRef, () => Math.floor(Math.random() * 100))

        return () => (
            <div>
                <span>controlledComputed:受另一个响应变量控制的computed</span><br />
                <span>ComputedRef:{comRef.value}</span><br />
                <button onClick={() => { sourceRef.value = Math.floor(Math.random() * 100) }}>生成随机数</button>
            </div>
        )
    }
})

const ExtendRef = defineComponent({
    setup() {
        const extended = extendRef(ref('d'), { a: 'a', b: 'b', c: ref('c') })
        return () => (
            <div>
                <span>extendRef:向响应式变量添加额外的属性,如果新添加的属性也是响应式的,修改此属性值将会响应到此属性的原始的响应变量,并将触发组件更新。如果不是响应式的那属性为只读属性</span><br />
                <span>扩展后多了a,b,c3个属性.其中c属性为Ref类型:{Reflect.ownKeys(extended).toString()}</span><br />
                <span>c属性:{extended.c}</span><br />
                <button onClick={() => { extended.c = Math.floor(Math.random() * 100) + '' }}>修改扩展添加的属性c(将触发更新)</button>
            </div>
        )
    }
})

const MakeDestructurable = defineComponent({
    setup() {

        const a = { value: 'a' }
        const b = 3
        const obj = makeDestructurable({ a, b } as const, [a, b] as const)
        const { a: a0, b: b0 } = obj
        const [a1, b1] = obj
        return () => (
            <div>
                <span>makeDestructurable:用于创建一个可以使用对象解构也可以使用数组解构的对象</span><br />
                <span>const {'{ a: a0, b: b0 }'} = obj</span><br />
                <span>const {'[a1, b1]'} = obj</span><br />
                <span>a0==a1:{(a0 == a1) + ''}</span><br />
                <span>b0==b1:{(b0 == b1) + ''}</span>
            </div>
        )
    }
})

const SyncRef = defineComponent({
    setup() {
        const [sourceRef, targetRef, targetRef2] = [ref(''), ref(''), ref('')]
        const stop = syncRef(sourceRef, [targetRef, targetRef2])
        return () => (
            <div>
                <span>syncRef:单向引用同步</span><br />
                <input type="text" value={sourceRef.value} onInput={e => { sourceRef.value = e.target.value }} placeholder='source' /><br />
                <input type="text" value={targetRef.value} onInput={e => { targetRef.value = e.target.value }} placeholder='target1' /><br />
                <input type="text" value={targetRef2.value} onInput={e => { targetRef2.value = e.target.value }} placeholder='target2' /><br />
                <button onClick={stop}>停止同步</button>
            </div>
        )
    }
...
const UseDebounce = defineComponent({
    setup() {
        const value = ref(0)
        const vuleDeb = useDebounce(value, 1000)
        return () => (
            <div>
                <span>useDebounce:把一个响应式变量包装返回一个新的与起初的响应式变量单向同步的(起初的响应式变量被修改时将防抖式的把数值同步过去)具有防抖功能的响应式变量,</span><br />
                <span>防抖功能的响应式变量:{vuleDeb.value}</span><br />
                <button onClick={() => { value.value = Math.floor(Math.random() * 100) + '' }}>修改起初的响应式变量</button>
            </div>
        )
    }
})

const UseDebounceFn = defineComponent({
    setup() {
        const valueRef = ref(0)
        const clickDebFn = useDebounceFn(() => { valueRef.value++ }, 1000)
        return () => (
            <div>
                <span>useDebounceFn:把函数包装成具有防抖功能</span><br />
                <span>count:{valueRef.value}</span><b></b>
                <button onClick={clickDebFn}>+1</button>
            </div>
        )
    }
})
...
export default defineComponent({
    ...
    components: {
        ...
        UseTimeout, UseTimeoutFn, AsyncComputed, UseAsyncState, UseRefHistory, BiSyncRef, ControlledComputed, ExtendRef, MakeDestructurable, SyncRef, UseDebounce,UseDebounceFn
    },
    setup() {
