...
const UseMouseInElement = defineComponent({
    setup() {
        const elRef = ref<Element>(null)
        function numToInt(obj: { [key: string]: Ref<any> }) {
            return new Proxy(obj, {
                get(target, propKey, receiver) {
                    if (typeof target[propKey].value === 'number') {
                        target[propKey].value = Math.round(target[propKey].value)
                    }
        ...
        const { x, y, sourceType, elementHeight, elementWidth, elementX, elementY, elementPositionX, elementPositionY, isOutside, stop } = numToInt(useMouseInElement(elRef))
