...
const UseDebounce = defineComponent({
    setup() {
        ...
        const vuleDeb = useDebounce(valueRef, 1000)
        return () => (
            <div>
                ...
                <button onClick={() => { valueRef.value = Math.floor(Math.random() * 100) + '' }}>修改起初的响应式变量</button>
            </div>
        )
    }
})
...
const UseThrottle = defineComponent({
    setup() {
        const valueRef = ref(0)
        const vuleThr = useThrottle(valueRef, 1000)
        return () => (
            <div>
                <span>useThrottle:把一个响应式变量包装返回一个新的与起初的响应式变量单向同步的(起初的响应式变量被修改时将节流式的把数值同步过去)具有节流功能的响应式变量,</span><br />
                <span>节流功能的响应式变量:{vuleThr.value}</span><br />
                <button onClick={() => { valueRef.value = Math.floor(Math.random() * 100) + '' }}>修改起初的响应式变量</button>
            </div>
        )
    }
...
const UseThrottleFn = defineComponent({
    setup() {
        const valueRef = ref(0)
        const clickThrFn = useThrottleFn(() => { valueRef.value++ }, 1000)
        return () => (
            <div>
...
const UseNProgress = defineComponent({
    setup(){
        const {} = useNProgress()
        return ()=>(
            <div>
                <span>useNProgress:</span><br/>
            </div>
        )
    }
})
