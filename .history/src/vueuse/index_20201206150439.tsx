...
import { useVModel, debouncedWatch, ignorableWatch, pausableWatch, throttledWatch, when, useCounter, useLocalStorage, useSessionStorage, useStorage, useMouse } from '@vueuse/core'
...
const UseStorage = defineComponent({
    setup() {
        const storage = useStorage('useStorage', 0, localStorage)
        watch(storage, () => {
            console.log(localStorage.getItem('useStorage'), sessionStorage.getItem('useStorage'))
        })
        return () => (
            <div>
...
export default defineComponent({
    name: 'VueuseStudy',
    components: { UseVModel, DebouncedWatch, IgnorableWatch, PausableWatch, UseMouse, When, UseCounter, CreateGlobalState, UseLocalStorage, UseSessionStorage, UseStorage },
    setup() {
