import { defineComponent, CSSProperties, ref, watch, unref } from 'vue'
import {
...
const UseMouseInElement = defineComponent({
    setup() {
        const elRef = ref<Element>(null)
        const { x, y, sourceType, elementHeight, elementWidth, elementX, elementY, elementPositionX, elementPositionY, isOutside, stop } = useMouseInElement(elRef)
        return () => (
            <div>
                <span>useMouseInElement:</span>
                <div ref={elRef} style={{ width: '200px', height: '200px', display: 'flex', flexDirection: 'column', alignItems: 'start', padding: '5px', backgroundColor: '#eaff8f' }}>
                    <span>x:{x.value}</span>
                    <span>y:{y.value}</span>
                    <span>sourceType:{sourceType.value}</span>
                    <span>elementX:{elementX.value}</span>
                    <span>elementY:{elementY.value}</span>
                    <span>isOutside:{isOutside.value + ''}</span>
                    <span>elementPositionX:{elementPositionX.value}</span>
                    <span>elementPositionY:{elementPositionY.value}</span>
                </div>
            </div>
        )
    }
