import { defineComponent, CSSProperties, ref, watch, nextTick } from 'vue'
import {
...
const OnStartTyping = defineComponent({
    setup() {
        ...
        nextTick(() => {
            onStartTyping(() => {
               // if (!elRef.value.active) 
                console.log('ppp')
                //elRef.value.focus()
            }, { document: elRef.value })
        })
        // onStartTyping(() => {
        //     if (!elRef.value.active)console.log('ppp')
        //         //elRef.value.focus()
        // },{document:elRef.value})
        return () => (
            <div>
                <span>onStartTyping:</span><br />
                <input ref={elRef} type="text" name="" id="" />
            </div>
        )
    }
})
...
export default defineComponent({
    ...
    components: {
        ...
        UseMouse, When, UseCounter, CreateGlobalState, UseLocalStorage, UseSessionStorage, UseStorage, UseToggle, OnClickOutside, OnStartTyping
    },
    setup() {
