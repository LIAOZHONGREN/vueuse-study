...
const UseRafFn = defineComponent({
    setup() {
        ...
        const { pause, resume } = useRafFn(() => { countRef.value++ }, { immediate: false })
        return () => (
            <div>
                <span>useRafFn:调用函数requestAnimationFrame。具有暂停和恢复的控制。</span><br />
                <h2 style={{margin:'5px 0'}}>count:{countRef.value}</h2>
                <button onClick={resume}>开始</button>
...
export default defineComponent({
    ...
    components: {
        ...
        UseMediaQuery, UseMutationObserver, UsePermission, UsePreferredLanguages, UseResizeObserver, UseTitle,UseRafFn
    },
    setup() {
