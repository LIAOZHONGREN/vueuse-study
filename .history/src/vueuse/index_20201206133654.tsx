...
import { useVModel, debouncedWatch, ignorableWatch, pausableWatch, throttledWatch, when, useCounter, useMouse } from '@vueuse/core'
...
const CreateGlobalState = defineComponent({
    setup() {
        ...
        import('./useGlobalState').then(module => {
            valueRef.value = module.useGlobalState().value
        })

        return () => (
            <div>
                <span>createGlobalState:创建供全局使用的状态</span>
                <div>useGlobalState().value:{valueRef.value}</div>
            </div>
        )
    }
})
...
export default defineComponent({
    name: 'VueuseStudy',
    components: { UseVModel, DebouncedWatch, IgnorableWatch, PausableWatch, UseMouse, When, UseCounter, CreateGlobalState },
    setup() {
