...
import {
    ...
    useBrowserLocation, useClipboard, useCssVar, useElementSize, useEventListener, useFavicon, useFullscreen, useMediaQuery, useMutationObserver
} from '@vueuse/core'
...
const UseMutationObserver = defineComponent({
    setup() {
        ...
        const { isSupported, stop } = useMutationObserver(elRef, (mutations, observer) => {
            console.log('oo')
           // logsRef.value.push(`发生变化的属性:${mutations[0].attributeName}`)
        }, { childList: true })//attributes:设置观察元素属性的变化
        return () => (
            <div>
                <span>useMutationObserver:</span><br />
                <input ref={elRef} type="radio" checked={checked.value} onClick={toggle} /><span>:点击我改变我的checked属性</span><br />
                <button onClick={() => { logsRef.value = [] }}>清除logs</button><br />
                <button onClick={() => { logsRef.value.push(Math.random() * 10) }}>添加log</button><br />
...
export default defineComponent({
    ...
    components: {
        ...
        UseParallax, UseWindowScroll, UseWindowSize, UseBrowserLocation, UseClipboard, UseCssVar, UseElementSize, UseEventListener, UseFavicon, UseFullscreen, UseMediaQuery, UseMutationObserver
    },
    setup() {
