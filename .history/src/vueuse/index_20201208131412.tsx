import { defineComponent, CSSProperties, ref, watch, unref, Ref, nextTick } from 'vue'
import {
    useVModel,
    ...
    useRafFn, useTimestamp, useTransition, TransitionPresets
} from '@vueuse/core'
    ...
    setup() {
        ...
        return () => (
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <button onClick={() => { targetNumRef.value = targetNumRef.value == 100 ? 0 : 100 }}>transition</button>
                <div style={{ position: 'relative', width: '300px', height: '20px', borderRadius: '10px', backgroundColor: 'rgba(0,0,0,0.3)' }}>
                    <div style={{ position: 'absolute', width: `${easeInOutCubic.value}%`, height: '20px', borderRadius: '10px', backgroundColor: '#d3f261' }}>
                        <span style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}>{easeInOutCubic.value}%</span>
                    </div>
                </div>
                <div style={{ width: '30px', height: '30px', backgroundColor: '#91d5ff', transform: `translateX(${easeInOutCubic.value}px) rotate(${270 * easeInOutBack.value / 100}deg)` }} />
            </div>
        )
    }
})
...
export default defineComponent({
    ...
    components: {
        ...
        UseMediaQuery, UseMutationObserver, UsePermission, UsePreferredLanguages, UseResizeObserver, UseTitle, UseRafFn, UseTimestamp, UseTransition
    },
    setup() {
