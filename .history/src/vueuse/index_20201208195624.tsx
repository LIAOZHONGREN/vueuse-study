...
const ExtendRef = defineComponent({
    setup() {
        ...
        const extended = extendRef(ref('c'), { a: 'a', b: 'b' })
        return () => (
            <div>
                <span>extendRef:</span><br />
                <span>扩展结果:{JSON.stringify(unref(extended))}</span><br />
                <button onClick={() => { extended.a = Math.floor(Math.random * 100) + '' }}>修改扩展添加是属性a(将触发更新)</button>
            </div>
        )
    }
})
