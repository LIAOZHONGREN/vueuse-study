    ...
    setup() {
        ...
        return () => (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'start', padding: '5px', backgroundColor: '#eaff8f', fontSize: '13px' }}>
                    <span>是否支持:{isSupported + ''}</span>
                    <span>是否在线:{isOnline.value + ''}</span>
                    <span>上次网络连接以来的时间:{new Date(offlineAt.value).toUTCString()}</span>
                    <span>下载速度:{downlink.value}Mbps</span>
                    <span>最大下载速度:{downlinkMax.value}Mbps</span>
...
export default defineComponent({
    ...
    components: {
        ...
        UseElementVisibility, UseGeolocation, UseIdle, UseIntersectionObserver, UseMouse, UseMouseInElement, UseNetwork
    },
    setup() {
