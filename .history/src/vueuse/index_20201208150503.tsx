...
const AsyncComputed = defineComponent({
    setup() {
        const num = ref(0)
        const com = asyncComputed(async () => {
            return await new Promise((resolve, reject) => { setTimeout(() => { console.log(num.value); resolve(num.value) }, 3000) })
        })
        return () => (
            <div>
                <span>asyncComputed:</span><br />
                <span>{com.value}</span><br />
                <button onClick={() => { num.value = Math.floor(Math.round() * 100) }}>随机</button>
            </div>
        )
    }
})
...
export default defineComponent({
    ...
    components: {
        ...
        UseTimeout, UseTimeoutFn, AsyncComputed
    },
    setup() {
