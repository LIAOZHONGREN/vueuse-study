...
const UseTimestamp = defineComponent({
    setup() {
        const { pause, resume, isActive, timestamp } = useTimestamp({offset:1000})
        return () => (
            <div>
                <span>useTimestamp:</span><br />
                <h2 style={{ margin: '3px 0' }}>Now:{timestamp.value}</h2>
            </div>
        )
    }
})
...
export default defineComponent({
    ...
    components: {
        ...
        UseMediaQuery, UseMutationObserver, UsePermission, UsePreferredLanguages, UseResizeObserver, UseTitle, UseRafFn,UseTimestamp
    },
    setup() {
