...
const UseAsyncState = defineComponent({
    setup() {
        const { state, ready } = useAsyncState<string>(new Promise(resolve => { setTimeout(() => resolve('ok'), 2000) }), 'await', 1000/**延时*/, err => { console.log(err) })
        return () => (
            <div>
                <span>useAsyncState:响应式异步状态。一旦完成，将不会阻止你的设置功能，并且会触发更改。</span><br />
                <span>ready:{ready.value + ''}</span>
...
export default defineComponent({
    ...
    components: {
        ...
        UseTimeout, UseTimeoutFn, AsyncComputed,UseAsyncState
    },
    setup() {
