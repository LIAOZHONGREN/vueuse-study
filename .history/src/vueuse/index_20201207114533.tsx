...
import {
    ...
    useDevicePixelRatio, useDocumentVisibility, useElementVisibility, useGeolocation, useIdle, useIntersectionObserver, useMouseInElement, useNetwork
} from '@vueuse/core'
    ...
    setup() {
        ...
        return () => (
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <span>useMouseInElement:返回与元素相关的反应性鼠标位置等信息</span><br/>
...
const UseNetwork = defineComponent({
    setup() {
        ...
        return () => (
            <div style={{ width: '200px', display: 'flex', flexDirection: 'column', alignItems: 'start', padding: '5px', backgroundColor: '#eaff8f', fontSize: '13px' }}>
                <span>useNetwork:反应性网络状态。网络信息API按照常规连接类型（例如，“ wifi”，“蜂窝”等）提供有关系统连接的信息。这可用于根据用户的连接选择高清晰度内容或低清晰度内容。</span><br />
                <span>是否支持:{isSupported + ''}</span><br />
                <span>是否在线:{isOnline.value + ''}</span><br />
                <span>上次网络连接以来的时间:{offlineAt.value}</span><br />
                <span>下载速度:{downlink.value}Mbps</span><br />
                <span>最大下载速度:{downlinkMax.value}Mbps</span><br />
                <span>网络速度类型:{effectiveType.value}</span><br />
                <span>是否激活了数据保护程序模式:{saveData.value + ''}</span><br />
