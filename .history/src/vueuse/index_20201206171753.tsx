...
const OnClickOutside = defineComponent({
    setup() {
        ...
        const styleRef = ref<CSSProperties>({ width: '100px', height: '100px', backgroundColor: '#d3f261', display: 'none' })
        onClickOutside(elRef, () => { styleRef.value.display = 'none' })
        return () => (
            <div>
                <span>onClickOutside:监听点击在提供元素以外的地方时执行事件函数</span>
                <div ref={elRef} style={styleRef.value}></div>
                <button disabled={styleRef.value.display !== 'none'} onClick={() => { styleRef.value.display = 'flex' }}>显示隐藏的元素</button>
            </div>
        )
    }
})
