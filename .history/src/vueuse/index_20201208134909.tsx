...
import {
    ...
    useRafFn, useTimestamp, useTransition, TransitionPresets, useInterval, useIntervalFn
} from '@vueuse/core'
...
const UseIntervalFn = defineComponent({
    setup() {
        ...
        const { pause, resume } = useIntervalFn(() => {
            const num = text.value.length % 6
            text.value = new Array(num + 1).fill('.').join('')
        }, 1000)
        return () => (
            <div>
                <span>useIntervalFn:指定间隔时间执行回调函数.提供开始,终止,暂停,恢复等控制函数</span><br />
                <h1 style={{ margin: '3px 0' }}>{text.value}</h3>
            </div>
        )
    }
})
