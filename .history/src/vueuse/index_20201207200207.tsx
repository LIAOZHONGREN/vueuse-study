...
import {
    ...
    useBrowserLocation, useClipboard, useCssVar, useElementSize, useEventListener, useFavicon
} from '@vueuse/core'
...
const UseFavicon = defineComponent({
    setup() {
        ...
        const icon = useFavicon(typeRef, { baseUrl: '@/assets/' })
        watch(typeRef, () => { console.log(typeRef.value) })
        return () => (
            <div>
                <span>useFavicon:</span>
                {/* <img src={icon.value} width={100} /> */}
                <button onClick={() => { typeRef.value = icons[(icons.indexOf(typeRef.value) + 1) % icons.length] }}>切换图表</button>
            </div>
        )
    }
})
...
export default defineComponent({
    ...
    components: {
        ...
        UseParallax, UseWindowScroll, UseWindowSize, UseBrowserLocation, UseClipboard, UseCssVar, UseElementSize, UseEventListener, UseFavicon
    },
    setup() {
