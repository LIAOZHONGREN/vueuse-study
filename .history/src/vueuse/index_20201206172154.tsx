...
const OnClickOutside = defineComponent({
    setup() {
        ...
        const styleRef = ref<CSSProperties>({ position: 'fixed', top: 'auto', left: 'auto', width: '100px', height: '100px', backgroundColor: '#d3f261', display: 'none' })
        onClickOutside(elRef, () => { styleRef.value.display = 'none' })
        return () => (
            <div>
                <span>onClickOutside:监听点击在提供元素以外的地方时执行事件函数</span><br />
                <div ref={elRef} style={styleRef.value}></div><br />
